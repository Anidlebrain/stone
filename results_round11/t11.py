...
def load_judge_module():
    ...
    ok = getattr(judge, "round_no", None) == 11 and hasattr(
        judge, "calculate_match_result")
    if ok:
        return judge
    try:
...
def unique_rows_preserve_first(arr: np.ndarray) -> np.ndarray:
    ...
    keys = arr16.view(
        np.dtype((np.void, arr16.dtype.itemsize * arr16.shape[1]))).ravel()
...
def gen_safe_under100(rng: np.random.Generator, n: int) -> np.ndarray:
    ...
    """
    alpha = np.array([0.7, 0.8, 0.9, 1.0, 1.0, 1.1, 1.2,
                     1.35, 1.55, 1.75], dtype=np.float64)
    ...
    for i in range(n):
        if rem[i] > 0:
            idx = rng.choice(K, size=int(rem[i]), replace=True, p=prob)
            np.add.at(x[i], idx, 1)
...
def gen_exact100(rng: np.random.Generator, n: int) -> np.ndarray:
    ...
    """
    alpha = np.array([0.8, 0.85, 0.9, 1.0, 1.0, 1.1, 1.25,
                     1.45, 1.65, 1.85], dtype=np.float64)
    p = rng.dirichlet(alpha, size=n)
    x = np.floor(p * 100).astype(np.int32)
    rem = 100 - x.sum(axis=1)
    prob = alpha / alpha.sum()
    for i in range(n):
        if rem[i] > 0:
            idx = rng.choice(K, size=int(rem[i]), replace=True, p=prob)
...
def gen_zero_trap(rng: np.random.Generator, n: int) -> np.ndarray:
    ...
    for i in range(n):
        ...
        if tiny_cnt > 0 and rest.size > 0:
            tidx = rng.choice(rest, size=min(
                tiny_cnt, rest.size), replace=False)
            x[i, tidx] = rng.integers(0, 4, size=tidx.shape[0])
        if rng.random() < 0.35:
            j = int(rng.choice(idx))
            x[i, j] = int(rng.choice(
                np.array([80, 90, 100, 110, 120, 130, 140, 150], dtype=np.int32)))
    return clip_strategies(x)
...
def gen_tail_focus(rng: np.random.Generator, n: int) -> np.ndarray:
    ...
    x[:, 5:] += rng.integers(8, 36, size=(n, 5), dtype=np.int32)
    for i in range(n):
        hot = int(rng.integers(1, 4))
        idx = rng.choice(np.arange(6, 10), size=hot, replace=False)
...
def gen_ladder_diff(rng: np.random.Generator, n: int) -> np.ndarray:
    ...
    而是只在若干关键堆上布置“挡板”。
    """
    x = rng.integers(0, 6, size=(n, K), dtype=np.int32)
    for i in range(n):
        start = int(rng.integers(4, 8))
        cnt = int(rng.integers(2, 5))
        idx = np.arange(start, min(K, start + cnt))
        first = int(rng.choice(
            np.array([85, 95, 100, 110, 120], dtype=np.int32)))
...
def gen_over100_controlled(rng: np.random.Generator, n: int) -> np.ndarray:
    ...
    这类策略会更敢拿关键堆，但不过分极端。
    """
    alpha = np.array([0.7, 0.8, 0.9, 0.95, 1.0, 1.1, 1.25,
                     1.45, 1.7, 1.95], dtype=np.float64)
    p = rng.dirichlet(alpha, size=n)
...
def gen_spikes(rng: np.random.Generator, n: int) -> np.ndarray:
    ...
    values = np.array([40, 50, 60, 70, 85, 100, 110, 120,
                      140, 160, 180], dtype=np.int32)
    for i in range(n):
        hot = int(rng.integers(1, 4))
        idx = rng.choice(K, size=hot, replace=False)
        x[i, idx] = rng.choice(values, size=hot)
    return clip_strategies(x)
...
def gen_manual_like(rng: np.random.Generator, n: int) -> np.ndarray:
    ...
    for i in range(n):
        if modes[i] == 0:
            ...
            ladder = np.array(
                [0, 1, 2, 4, 7, 11, 16, 22, 29, 37], dtype=np.int32)
            noise = rng.integers(-3, 4, size=K)
            x[i] = ladder + noise
    return clip_strategies(x)


def gen_random_uniform(rng: np.random.Generator, n: int) -> np.ndarray:
    return rng.integers(0, 101, size=(n, K), dtype=DTYPE)


def generate_batch_mixed(rng: np.random.Generator, batch_size: int) -> tuple[np.ndarray, dict]:
    spec = [
        ("safe_under100", 0.18, gen_safe_under100),
        ("exact100", 0.16, gen_exact100),
        ("zero_trap", 0.14, gen_zero_trap),
        ("tail_focus", 0.14, gen_tail_focus),
        ("ladder_diff", 0.08, gen_ladder_diff),
        ("over100", 0.08, gen_over100_controlled),
        ("spikes", 0.06, gen_spikes),
        ("manual_like", 0.06, gen_manual_like),
        ("random", 0.10, gen_random_uniform),
    ]
    counts = {}
    remain = batch_size
    arrs = []
...
def generate_unique_strategies(N: int, seed: int, log_every: int, batch_size: int) -> np.ndarray:
    ...
    while inserted < N:
        batch, counts = generate_batch_mixed(
            rng, min(batch_size, (N - inserted) * 2))
        ...
        if log_every and inserted % log_every == 0:
            dt = time.time() - t0
            dup_rate = 1.0 - inserted / \
                max(trials + len(additional_strategies), 1)
            print(
                f"[gen] {inserted:,}/{N:,} unique | batch={batch_size:,} | approx_dup={dup_rate:.4f} | {dt:.1f}s")
...
def save_stage(out_dir: str, name: str, strats: np.ndarray, wins: np.ndarray):
    ...
    np.savez_compressed(path, strategies=strats.astype(
        DTYPE), wins=wins.astype(np.int32))
...
def parse_args():
    p = argparse.ArgumentParser(description="第11回合沙堆养蛊多级筛选（多样性增强版）")
    ...
    p.add_argument("--out_dir", type=str,
                   default=getattr(judge, "name", "results_round11"))
...
def main():
    ...
    strats0 = generate_unique_strategies(
        args.n0, seed=args.seed_gen, log_every=args.gen_log_every, batch_size=args.batch_size)
    ...
    _ = stage_run(strats3, args.n4, args.mode4, args.rounds4,
                  args.seed4, args.log_every_rounds, args.out_dir, "top100")
