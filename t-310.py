...
ALL_LENGTHS = [tuple(x) for x in itertools.product(range(1, K + 1), repeat=GROUPS) if sum(x) == K]
...
def is_legal_round10(v: np.ndarray) -> bool:
    return bool(v.shape[0] == K and (v >= 0).all() and int(v.sum()) == TOTAL and exists_round10_partition(v))
...
def decode_state(lengths, sums, masks) -> np.ndarray:
    ...
    for m, S, mask in zip(lengths, sums, masks):
        ...
        q = S // m
        r = S % m
        seg = np.full(m, q, dtype=np.int16)
        if r > 0:
            for idx in mask:
                seg[int(idx)] += 1
        x[pos:pos + m] = seg
...
def state_from_vector(v: np.ndarray, rng: np.random.Generator | None = None) -> State:
...
def random_state(rng: np.random.Generator) -> State:
    lengths = tuple(int(v) for v in random_positive_composition(K, GROUPS, rng))
    sums = tuple(int(v) for v in random_nonnegative_composition(TOTAL, GROUPS, rng))
    masks = []
...
def global_search(pool, restarts=30, iters=3000, neighborhood=40, max_step=8, seed=1, verbose=True):
    ...
    for r in range(1, restarts + 1):
        ...
        if bucket < 4 and pool.shape[0] > 0:
            ...
            x0 = mutate_state(state_from_vector(x0, rng), rng, max_coin_step=5).decode()
        elif bucket < 8:
        ...
        if verbose:
            dt = time.time() - t0
            print(f"[restart {r:3d}/{restarts}] best_this={w}/{t}/{l} | global_best={global_w}/{global_t}/{global_l} | x={global_best.tolist()} | {dt:.1f}s")
...
def main():
    ...
    print(f"Pool size = {pool.shape[0]:,} (legal / raw = {pool.shape[0]:,} / {raw_n:,})")
    ...
    with open(args.save, "a", encoding="utf8") as f:
        f.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} round10-compact wins={best_w} ties={best_t} losses={best_l} win_rate={best_w / pool.shape[0]:.6f}\n")
