...
(applies only for that next pile; can trigger again on later piles).
...
def parse_args():
    p = argparse.ArgumentParser(description="Round-4 沙堆养蛊多级筛选")

    # Generation
    p.add_argument("--seed_gen", type=int, default=1)
    p.add_argument("--n0", type=int, default=1_000_000, help="initial unique strategies count (default 1,000,000)")
    p.add_argument("--gen_log_every", type=int, default=50_000, help="generation log frequency")

    # Output directory
    p.add_argument("--out_dir", type=str, default="results_round4")
    ...
    # Logging
    p.add_argument("--log_every_rounds", type=int, default=10, help="tournament log every X rounds")
